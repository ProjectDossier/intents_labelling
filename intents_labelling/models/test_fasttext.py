...
def save_test(data_path,df,col_name):
    df[["query", col_name]].to_csv(data_path,index=False,sep=' ',header=None,quoting=csv.QUOTE_NONE,quotechar="",escapechar=" ")

def prepare_test(df,col_name):
    df = df[["query", col_name]]
    df = df.fillna('')
...
def prediction_labels(model,df,col_name):
    test_preds = []
    for row in df["query"].tolist():
        test_preds.append(model.predict(row)[0][0])
    df['test_prediction'] = test_preds
    res = df[col_name].to_numpy()
...
if __name__ == "__main__":
    ...
    pl = prediction_labels(model,pt,"label_man")
    print(pl[0])
